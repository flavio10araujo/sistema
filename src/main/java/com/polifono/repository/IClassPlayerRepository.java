...
public interface IClassPlayerRepository extends JpaRepository<ClassPlayer, Integer> {
    ...
    @Query("SELECT classPlayer FROM ClassPlayer classPlayer, com.polifono.model.Class clazz, Player player WHERE classPlayer.clazz.id = clazz.id AND classPlayer.player.id = player.id AND clazz.player.id = :playerId AND classPlayer.clazz.id = :clazzId AND (classPlayer.status = 1 OR classPlayer.status = 2) ORDER BY player.name ASC")
    ...
    @Query("SELECT classPlayer FROM ClassPlayer classPlayer, com.polifono.model.Class clazz WHERE classPlayer.clazz.id = clazz.id AND clazz.player.id = :playerId AND (classPlayer.status = 1 OR classPlayer.status = 2)")
    ...
    @Query("SELECT classPlayer FROM ClassPlayer classPlayer, com.polifono.model.Class clazz WHERE classPlayer.clazz.id = clazz.id AND clazz.player.id = :teacherId AND classPlayer.player.id = :studentId AND classPlayer.status = 2")
